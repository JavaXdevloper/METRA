...
type Inspection = { id: string; productId: string; product?: string; manufacturer?: string; createdAt?: string; date?: string; complianceStatus: Status; status?: Status; images: string[]; extractedDeclarations?: Record<string, string>; declarations?: Declaration[]; violations: Violation[]; userId?: string; officer?: string; location?: string; confidence?: number; ocrData?: { confidence: number } };
...
function StatusBadge({ status }: { status?: Status }) { const m = status ? (statusMeta[status] || statusMeta['NEEDS_REVIEW']) : statusMeta['NEEDS_REVIEW']; const Icon = m.icon; return <span className={`status-badge ${m.className}`}><Icon size={13} />{m.label}</span>; }
...
function InspectionTable({ rows = [] }: { rows?: Inspection[] }) { return <div className="table-wrap"><table><thead><tr><th>Inspection ID</th><th>Product / manufacturer</th><th>Recorded</th><th>Status</th><th></th></tr></thead><tbody>{rows.map((item) => <tr key={item.id}><td><Link href={`/inspection/${item.id}`} className="table-id"><Mono>{item.id}</Mono></Link></td><td><div className="product-cell"><strong>{item.product}</strong><small>{item.manufacturer}</small></div></td><td><span className="muted">{item.date || item.createdAt}</span></td><td><StatusBadge status={item.status || item.complianceStatus} /></td><td><Link href={`/inspection/${item.id}`} className="row-arrow"><ArrowRight size={17} /></Link></td></tr>)}</tbody></table></div>; }
function Dashboard() { 
